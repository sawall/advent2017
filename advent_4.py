...
def day4(part2=False):
    ...
    for line in data.split('\n'):
        if not part2:
            ...
            wordlist = [''.join(sorted(word)) for word in line.split()]
        if wordlist != [] and len(wordlist) == len(set(wordlist)):
            validphrases += 1
    print('part ' + ('two' if part2 else 'one') + ': valid passphrases: ' + str(validphrases))
